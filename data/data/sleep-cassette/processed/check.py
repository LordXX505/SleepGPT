...
import os
import glob
def check_mul(train, test, test_nums):
...
def check(mode='mul'):
    ...
    downstream_file = np.load(os.path.join(args.output_dir, f"edf_downstream_{mode}.npy"), allow_pickle=True).item()
    for random_seed in range(0, 2):
        print(f'randomseed : {random_seed}, mode: {mode}')
        ...
        test_2013_nums = 0
        if mode == 'mul':
            check_mul(down_stream_train_res, down_stream_test_res, downstream_file[f'test_{random_seed}']['nums'])
        elif mode == 'usleep':
        ...
        for trn in down_stream_train_res:
            ...
            if int(number) <= 4192:
                train_2013_nums += 1
        print(train_2013_nums, test_2013_nums)
