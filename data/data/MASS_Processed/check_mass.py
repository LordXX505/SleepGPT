...
def main():
    ...
    for SSNum in range(1, 6):
        file[f'SS{SSNum}'] = np.load(os.path.join(args.data_dir, f'SS{SSNum}/split_k_20_SS{SSNum}.npy'), allow_pickle=True).item()
    ...
    for i in range(20):
        ...
        for vn, testn in zip(res_val_name, res_test_name):
            assert vn not in res_train_name
            assert testn not in res_train_name
...
def check_length():
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", type=str,
                        default="/Volumes/T7 Shield/MASS_Processed",
                        help="File path to the Sleep-MASS dataset.")
    parser.add_argument("--output_dir", type=str, default="processed",
                        help="Directory where to save outputs.")
    parser.add_argument("--log_file", type=str, default="info_extract.log",
    ...
    for ssnum in range(1, 6):
        ...
        for name in names:
            if os.path.basename(name) == '01-01-0012':
