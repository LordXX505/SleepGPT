...
def process_shhs(idx, path_list, anno_list):
    ...
    for i in tqdm(range(len(anno_list))):
        ...
        print('Remove movement and unknown stages if any')
        data = raw.get_data()
        if print_ is False:
            print(raw.ch_names)
        ...
        x = x[select_idx]
        y = y[select_idx]
        filename = '/root/DATA/data/shhs_new'
        cnt = 0
...
if __name__ == '__main__':
    ...
    p = Pool(16)
    Root_path = '/root/shhs/polysomnography/edfs'
    ...
    all_data_path_list = shhs2_data_path_list
    anna_root_path = '/root/shhs/polysomnography/annotations-events-profusion'
    ...
    all_anno_path_list = anna_shhs2_path_list
    ...
    result = []
    #
    piece = len(all_data_path_list)//16
    for i in range(16):
        start = i*piece
        if i == 15:
