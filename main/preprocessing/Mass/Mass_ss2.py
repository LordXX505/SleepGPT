...
epoch = os.path.join(path_root, 'bio')
def plot_generate(generate_epoch, res, batch_epoch, aug_time, i, save_label, inverse, name, idx):
    fig, Axes = plt.subplots(nrows=2, ncols=1, sharex='all', figsize=(30, 16))
    for c in range(1):
        Axes[0].plot(generate_epoch[c].detach().numpy())
        Axes[0].plot(save_label*generate_epoch[c].detach().numpy().max())
        Axes[0].set_xticks(np.arange(0, 2000, 200))
        Axes[1].plot(batch_epoch[c].detach().numpy() / 1e6)
        Axes[1].plot(save_label*generate_epoch[c].detach().numpy().max())
