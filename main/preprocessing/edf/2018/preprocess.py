...
def main():
    ...
    for i in range(len(psg_fnames)):
        ...
        logger.info("Loading ...")
        logger.info("Signal file: {}".format(psg_fnames[i]))  # read the raw data
        logger.info("Annotation file: {}".format(ann_fnames[i]))  # read the annotation

        psg_f = pyedflib.EdfReader(psg_fnames[i])
        ann_f = pyedflib.EdfReader(ann_fnames[i])

        assert psg_f.getStartdatetime() == ann_f.getStartdatetime()  # check the start time
        start_datetime = psg_f.getStartdatetime()  # return datetime(1989,4,24,16,13)
        logger.info("Start datetime: {}".format(str(start_datetime)))

        file_duration = psg_f.getFileDuration()  # duration:79500 second
        logger.info("File duration: {} sec".format(file_duration))
        epoch_duration = psg_f.datarecord_duration  # epoch_duration:30.0
        ...
        ch_names = psg_f.getSignalLabels()  # list channels
        ch_samples = psg_f.getNSamples()  # freq*time, every channel:[7950*100]*channels , EMG is 1hz
        select_ch_idx = -1
        for s in range(psg_f.signals_in_file):
            if ch_names[s] == select_ch:  # Fpz-cz
                select_ch_idx = s
                break
        if select_ch_idx == -1:
        ...
        sampling_rate = 100.0
        n_epoch_samples = int(epoch_duration * sampling_rate)  # 3000
        signals = psg_f.readSignal(select_ch_idx).reshape(-1, n_epoch_samples)  # 2650x 3000, epoch*samples
        ...
        labels = []
        total_duration = 0
        ann_onsets, ann_durations, ann_stages = ann_f.readAnnotations()  # ann_onsets =154 (just one example)
        for a in range(len(ann_stages)):
            onset_sec = int(ann_onsets[a])
            duration_sec = int(ann_durations[a])  # 30603s
            ...
            # Get label value
            label = ann2label[
                ann_str]  # {'Movement time': 5, 'Sleep stage 1': 1, 'Sleep stage 2': 2, 'Sleep stage 3': 3, 'Sleep stage 4': 3, 'Sleep stage ?': 6, 'Sleep stage R': 4, 'Sleep stage W': 0}
            ...
            duration_epoch = int(duration_sec / epoch_duration)  # 1021=30630/30

            # Generate sleep stage labels
            label_epoch = np.ones(duration_epoch, dtype=np.int32) * label  # 1021
            labels.append(label_epoch)  # list

            total_duration += duration_sec  # total_duration

            logger.info("Include onset:{}, duration:{}, label:{} ({})".format(
                onset_sec, duration_sec, label, ann_str
            ))
        labels = np.hstack(labels)  # labels = list:154 == len(ana) and transformer to 2880,154*30

        # Remove annotations that are longer than the recorded signals
        labels = labels[:len(signals)]  # labels contains label for one epoch
