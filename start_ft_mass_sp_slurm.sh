...
srun python3 main_kfold.py with finetune_MASS_Spindle   \
  num_gpus=2 num_nodes=1 num_workers=12 batch_size=128 model_arch=backbone_large_patch200  lr_mult=20 \
  warmup_lr=0 val_check_interval=0.5 check_val_every_n_epoch=1 limit_train_batches=1.0 max_steps=-1 all_time=True time_size=1 pool=None \
  ...
  load_path=$load_path \
  use_all_label='all' \
  optim="Lion" weight_decay=0.05 \
  layer_decay=0.65 get_param_method='no_layer_decay' Lambda=10.0 patch_size=200 use_cb=True kfold=5 \
  expert='E1' IOU_th=0.2 sp_prob=0.55 patch_time=20 use_fpfn='BCE' Use_FPN='Trans' Spindle_decoder_depth=4 Spindle_enc_dim=384 \
  mass_aug_times=2 \
