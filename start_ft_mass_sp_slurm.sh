...
srun python3 main_kfold.py with finetune_MASS_Spindle   \
  ...
  lr=1.25e-4 min_lr=5e-8 random_choose_channels=8 max_epoch=100 lr_policy=cosine loss_function='l1' drop_path_rate=0.2 warmup_steps=0.1 split_len=1 \
  load_path=$load_path \
  use_all_label='all' \
  ...
  expert='E2' IOU_th=0.2 sp_prob=0.55 patch_time=20 use_fpfn='Fpfn' Use_FPN='MLP' Spindle_decoder_depth=2 Spindle_enc_dim=1024 \
  grad_name='all' num_queries=250 FPN_resnet=False
