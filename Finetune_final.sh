...
OMP_NUM_THREADS=1 torchrun \
  ...
  lr=1e-4 min_lr=0 random_choose_channels=4 max_epoch=5 lr_policy=cosine loss_function='l1' drop_path_rate=0.5 warmup_steps=0.1 split_len=20 \
  load_path=$load_path \
  use_pooling='swin' use_relative_pos_emb=False  mixup=0 smoothing=0.1 decoder_heads=16  use_global_fft=True use_all_label='all' \
  use_multiway="multiway" use_g_mid=False get_param_method='layer_decay'  local_pooling=False optim="adamw" poly=False extra_name="Swin_final" weight_decay=0.05 \
  num_encoder_layers=6 layer_decay=0.5 Lambda=1.0 patch_size=200 use_cb=True